...
def read_timeline_events(skip: int = 0, limit: int = 100, tags: str = None, q: str = None, db: Session = Depends(get_db)):
    ...
    if q:
        events = [event for event in events if q.lower() in event.title.lower() or q.lower() in event.description.lower()]
    return events
